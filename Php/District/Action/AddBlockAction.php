...
if (isset($_POST['fname'])) {
	...
	if (mysqli_num_rows($result) > 0) {
		header("Location: ../AddBlockCor.php?error=The Email Id is taken try another");
		exit();
	}
	...
	if ($pin == null && $pin == "") {
		...
		if (mysqli_num_rows($result) === 1) {
			...
			if ($result) {
				...
				header("Location: ../AddBlockCor.php?error=Invalid Pin.");
				exit();
			}
		} else {
			header("Location: ../AddBlockCor.php?error=Invalid Pin.");
	...
	if (isset($_POST["submit"])) {
		...
		if ($check !== false) {
			...
			$uploadOk = 0;
			header("Location: ../AddBlockCor.php?error=File is not an image.");
			exit();
		}
	}
	...
	if ($_FILES["doc"]["size"] > 500000) {
		$uploadOk = 0;
		header("Location: ../AddBlockCor.php?error=Sorry, your file is too large.");
		exit();
	}
	...
	if ($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg") {
		$uploadOk = 0;
		header("Location: ../AddBlockCor.php?error=Sorry, only JPG, JPEG, PNG files are allowed..");
		exit();
	}

	// Check if $uploadOk is set to 0 by an error
	if ($uploadOk == 0) {
		header("Location: ../AddBlockCor.php?error=Sorry, your file was not uploaded.");
		exit();
		...
		if (move_uploaded_file($source, $destination)) {
			...
			header("Location: ../AddBlockCor.php?error=Sorry, there was an error uploading your file.");
			exit();
		}
	}
	...
	if ($result2) {
		header("Location: ../AddBlockCor.php?success=District Coordinator created successfully");
		exit();
	} else {
		printf("Errormessage: %s\n", mysqli_error($conn));
		header("Location: ../AddBlockCor.php?error=unknown error occurred" + mysqli_error($conn));
		exit();
	}
} else {
	header("Location: ../AddBlockCor.php");
	exit();
}
