	...
	if (mysqli_num_rows($result) > 0) {
		...
		if ($pin == null && $pin == "") {
			...
			if (mysqli_num_rows($result) === 1) {
				...
				if ($result) {
					...
					header("Location: ../AddDistrictCor.php?error=Invalid Pin.");
					exit();
				}
			} else {
				header("Location: ../AddDistrictCor.php?error=Invalid Pin.");
		...
		if (isset($_POST["submit"])) {
			...
			if ($check !== false) {
				...
				$uploadOk = 0;
				header("Location: ../AddDistrictCor.php?error=File is not an image.");
				exit();
			}
		}
		...
		if ($_FILES["doc"]["size"] > 500000) {
			$uploadOk = 0;
			header("Location: ../AddDistrictCor.php?error=Sorry, your file is too large.");
			exit();
		}
		...
		if ($imageFileType != "jpg" && $imageFileType != "png" && $imageFileType != "jpeg") {
			$uploadOk = 0;
			header("Location: ../AddDistrictCor.php?error=Sorry, only JPG, JPEG, PNG files are allowed..");
			exit();
		}

		// Check if $uploadOk is set to 0 by an error
		if ($uploadOk == 0) {
			header("Location: ../AddDistrictCor.php?error=Sorry, your file was not uploaded.");
			exit();
			...
			if (move_uploaded_file($source, $destination)) {
				...
				header("Location: ../AddDistrictCor.php?error=Sorry, there was an error uploading your file.");
				exit();
			}
		}
		...
		$sql2 = "INSERT INTO register(FNAME, LNAME, EMAIL, NUMBER, PASSWORD, DOB, ADDRESS1, ADDRESS2, ADDRESS3, PIN, USERTYPE, STATE, DISTRICT, BLOCK, GP, IDENTITY, IDNUMBER, PHOTO,ACTIVE,CREATEDON) 
				               VALUES('$fname','$lname','$email','$number','$password','$date','$address1','$address2','$address3','$pincode','$position','$state','$district','$block','$gp','$document','$dnumber','$basename','$active','$currdate')";
